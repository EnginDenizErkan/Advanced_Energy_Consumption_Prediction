...
st.set_page_config(
    page_title="Low Income Homes Prediction Page",
    page_icon="📈",
    layout="wide",
)
...
st.header("Low Income Homes Predictions")
st.write("""A family earning between £15,000 and £25,000 per year is considered a low-income household. Using energy consumption readings for a sample of 5,567 London Households that took part in the Low Carbon London project between November 2011 and February 2014, an energy consumption prediction for low-income households is made, and its results are represented on this page.""")
...
prediction = get_data(query="adversity_home_predictions")
...
average_income_values = metrics.loc[metrics['income_type'] == 'low_income']
...
with col3_metric:
    ...
    data=csv,
    file_name='Low_Income_Prediction.csv',
    mime='text/csv',
)
...
# Images
image_all_homes_depence = gui_utils.getImage("shap_dependence_plot_adversity.png")
image_all_homes_summary = gui_utils.getImage("shap_summary_plot_adversity.png")
image_all_homes_interaction_summary = gui_utils.getImage("shap_interaction_summary_plot_adversity.png")
