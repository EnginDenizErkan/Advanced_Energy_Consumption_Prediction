...
st.set_page_config(
    page_title="High Income Homes Prediction Page",
    page_icon="📈",
    layout="wide",
)
...
st.header("High Homes Predictions")
st.write("""A family earning more than £50,000 per year is considered a high-income household. Using energy consumption readings for a sample of 5,567 London Households that took part in the Low Carbon London project between November 2011 and February 2014, an energy consumption prediction for high-income households is made, and its results are represented on this page.""")
...
prediction = get_data(query="affluent_home_predictions")
...
average_income_values = metrics.loc[metrics['income_type'] == 'high_income']
...
with col3_metric:
    ...
    data=csv,
    file_name='High_Income_Prediction.csv',
    mime='text/csv',
)
...
# Images
image_all_homes_depence = gui_utils.getImage("shap_dependence_plot_affluent.png")
image_all_homes_summary = gui_utils.getImage("shap_summary_plot_affluent.png")
image_all_homes_interaction_summary = gui_utils.getImage("shap_interaction_summary_plot_affluent.png")
